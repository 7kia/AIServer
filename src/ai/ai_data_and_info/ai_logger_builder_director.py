...
class AiLoggerBuilderDirector:
    ...
    def create_ai_logger(self, ai_info: AiInfo, game_info: GameInfo):
        try:
            ai_logger = self._builder.create_ai_logger()
            ai_logger.end_game_state_file = self._builder.create_end_game_state_file(
                ai_info.ai_address, game_info.ai_options.troopType
            )
            ai_logger.game_state_log_file = self._builder.create_game_state_log_file(
                ai_info.ai_address, game_info.ai_options.troopType
            )
            return ai_logger
