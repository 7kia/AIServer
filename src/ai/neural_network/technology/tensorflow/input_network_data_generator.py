...
class InputNetworkDataGenerator:
    ...
    def generate_input_data(self,
                            ...
                            current_game_state: GameState) -> tf.data.Dataset:
        dataset_data: DatasetData = cls._generate_dataset_data(
            unit_observation, current_game_state
        )
        output_types = {k: tf.float32 for k in dataset_data.input_data}
        self.data = dataset_data.input_data
        dataset: tf.data.Dataset = tf.data.Dataset.from_generator(
            cls._get_input_data_generator,
            output_types=output_types,
            output_shapes=dataset_data.input_data_shapes
        )
        for element in dataset:
            element.name = tf.identity(element, name="my_new_tensor") #dataset_data.input_data
        return dataset
        ...
        for data_set in [unit_observation_data, person_unit_params_data, sector_params_data]:
            for key in data_set.keys():
                ...
                if isinstance(add_value, (float, int)):
                    result.input_data[key] = data_set[key]
                    result.input_data_shapes[key] = (None, 1)
                elif isinstance(add_value, list):
                    result.input_data[key] = cls._handle_list_or_matrix(add_value)
                    result.input_data_shapes[key] = (None, len(result.input_data[key]))
        return result
